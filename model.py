...
def read_records(index=0):
    train_queue = tf.train.string_input_producer(
        ['training.tfrecords'], num_epochs=FLAGS.epochs)
    ...
    document_batch_serialized, query_batch_serialized, answer_batch =\
    ...
    document_weights = tf.sparse_to_dense(
        sparse_document_batch.indices, sparse_document_batch.dense_shape, 1)

    query_batch = tf.sparse_tensor_to_dense(sparse_query_batch)
    query_weights = tf.sparse_to_dense(
        sparse_query_batch.indices, sparse_query_batch.dense_shape, 1)

    return document_batch, document_weights,\
        query_batch, query_weights, answer_batch
...
def inference(documents, doc_mask, query, query_mask):
    ...
    with tf.variable_scope('document', initializer=orthogonal_initializer()):
        ...
        h, _ = tf.nn.bidirectional_dynamic_rnn(
            ...
            sequence_length=tf.to_int64(doc_len), dtype=tf.float32)
    ...
    #    tf.reduce_mean(beta, reduction_indices=1), -1)
    ...
    y_hat = tf.stack([tf.unsorted_segment_sum(
        attentions, sentence_ids, FLAGS.vocab_size)
        for (attentions, sentence_ids) in unpacked_s])

    return y_hat, regularizer, M, alpha, beta, query_importance, s


def train(y_hat, regularizer, document, doc_weight, answer):
    # Trick while we wait for tf.gather_nd
    # https://github.com/tensorflow/tensorflow/issues/206
    # This unfortunately causes us to expand a sparse tensor
    # into the full vocabulary
    index = tf.range(0, FLAGS.batch_size) * \
        FLAGS.vocab_size + tf.to_int32(answer)
    ...
    accuracy = tf.reduce_mean(tf.to_float(
        tf.equal(tf.argmax(y_hat, 1), answer)))
    ...
    with tf.Session() as sess:
        ...
        if not FLAGS.training:
            saver_variables = filter(
                lambda var:
                    var.name != 'input_producer/limit_epochs/epochs:0',
                saver_variables)
            saver_variables = filter(
                lambda var: var.name != 'smooth_acc:0', saver_variables)
            saver_variables = filter(
                lambda var: var.name != 'avg_acc:0', saver_variables)
        # print('TYPE:', type(saver_variables))
        # print('TYPE:', type(list(saver_variables)))
        saver_variables = list(saver_variables)
